...
export const Sports = () => {
  ...
  useEffect(() => {
    const fetchZipcode = async () => {
      try {
        ...
        fetchSportsEvents(userZipCode);
      } catch (error) {
        console.log(error);
        fetchSportsEvents(zipCode);
      }
    };
    fetchZipcode();
  }, []);
  ...
  // Fetch sports events based on the coordinates
  const fetchSportsEvents = async (zip) => {
    setLoading(true);
    try {
      ...
      const response = await axios.get(
        `https://app.ticketmaster.com/discovery/v2/events.json?apikey=${apikey}&latlong=${lat},${lng}&classificationName=sports&size=50`
      );

      const sports = response.data._embedded?.events || [];

      // Sort the sports events by date
      const sortedSportsEvents = sports.sort((a, b) => {
        const dateA = new Date(a.dates.start.localDate);
        const dateB = new Date(b.dates.start.localDate);
        return dateA - dateB;
      });

      setSportsEvents(sortedSportsEvents);
    } catch (error) {
  ...
  return (
    <div className="homepage">
      {zipCode ? (
        <h1>Sports events near you</h1>
      ) : (
        <h1>Sports events near Chicago</h1>
      )}
      <div className="each-slide-container">
        <div className="each-slide1" ref={sportsRef}>
          {loading ? (
